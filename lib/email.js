...
export async function sendEmail({ to, subject, html }) {
  // [E2E-TEST] Trace what's being sent (remove after tests pass)
  console.log(`[email:trace] to=${to} subject=${subject}`);
...
<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  ...
  <p style="font-size:12px;color:#888">
    Or copy this link into your browser:<br/>
	    <a href="${safeVerifyUrl}" style="color:#e8a84c">${safeVerifyUrl}</a>
  </p>
...
export function buildRecipientInviteEmail({ writerName, readerName, isTrusted }) {
  const safeWriterName = escapeHtml(writerName);
  const safeReaderName = escapeHtml(readerName);
...
<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  ...
  <hr style="border:none;border-top:1px solid #e8a84c;margin-bottom:24px"/>
	  ...
	  <p>Hello ${safeReaderName},</p>

  <p>
	    <strong>${safeWriterName}</strong> has added you as a recipient in their private Letter Vault —
  ...
  `}
	  ...
	  <div style="background:#fffdf5;border:1px solid #e8a84c;border-radius:8px;padding:20px 24px;margin:28px 0">
	    <p style="margin:0 0 6px;font-size:12px;text-transform:uppercase;letter-spacing:.1em;color:#888">Your access details</p>
		    <p style="margin:0 0 4px;font-size:15px"><strong>Name:</strong> ${safeReaderName}</p>
		    <p style="margin:0;font-size:15px">Your passcode will be provided by the letter creator through a separate trusted channel.</p>
	  </div>

	  <p style="font-size:13px;color:#555">
	    Keep your passcode safe — you will need it to access your letters.
  ...
  <p style="font-size:12px;color:#999;margin-top:32px">
	    Letter Vault is a private posthumous letter system. Your details were added by ${safeWriterName}.
  </p>
</div>`,
  };
}
...
export function buildPreviewEmail({ writerName, sectionTitle, sectionText }) {
  ...
  const cleanText = sanitizeHtml(sectionText).replace(/src="data:[^"]*"/g, 'src=""');
  const safeWriterName = escapeHtml(writerName);
...
<div style="font-family:Georgia,serif;max-width:600px;margin:0 auto;color:#1a1a1a">
  ...
  <p style="font-size:12px;color:#888;border:1px solid #daa520;border-radius:6px;padding:10px 14px;background:#fffbf0">
    ⚠ This is a <strong>preview</strong> sent to you as the creator. Recipients will see this section after the vault unlocks.
	  ...
	  <h2 style="font-size:24px;margin:24px 0 8px">${safeSectionTitle}</h2>
  <div style="font-size:15px;line-height:1.85;color:#333">
    ${cleanText}
  </div>
  <hr style="border:none;border-top:1px solid #eee;margin:32px 0 16px"/>
	  <p style="font-size:12px;color:#999">Preview requested by <strong>${safeWriterName}</strong> · Letter Vault</p>
</div>`,
  };
}
...
<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  ...
  <hr style="border:none;border-top:1px solid #e8a84c;margin-bottom:24px"/>
	  <p>Hello ${safeReaderName},</p>
	  <p><strong>${safeWriterName}</strong> scheduled a letter section to be delivered to you today.</p>
...
<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  ...
  <p style="font-size:12px;color:#888">
    Or copy this link:<br/>
	    <a href="${safeResetUrl}" style="color:#e8a84c">${safeResetUrl}</a>
  </p>
  <p style="font-size:12px;color:#888;margin-top:32px">
...
<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  ...
  <hr style="border:none;border-top:1px solid #e8a84c;margin-bottom:24px"/>
	  <p>Hello ${safeWriterName},</p>
	  <p>Your email address has been updated to <strong>${safeNewEmail}</strong>.</p>
...
export function buildWarningEmail({ writerName, heartbeatToken, graceDays }) {
  ...
  const safeWriterName = escapeHtml(writerName);
  const safeHeartbeatUrl = escapeHtml(heartbeatUrl);
  const safeGraceDays = Number(graceDays);
  return {
...
<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  ...
  <p>
	    <strong>If we do not hear from you within ${safeGraceDays} day${safeGraceDays === 1 ? '' : 's'},
    your vault will automatically unlock and your letters will be delivered to your recipients.</strong>
  </p>
  <p style="margin:32px 0">
	    <a href="${safeHeartbeatUrl}"
       style="background:#e8a84c;color:#1a1209;padding:14px 28px;text-decoration:none;border-radius:6px;font-weight:bold;display:inline-block">
  ...
  <p style="font-size:12px;color:#888">
    Or copy this link into your browser:<br/>
	    <a href="${safeHeartbeatUrl}" style="color:#e8a84c">${safeHeartbeatUrl}</a>
  </p>
  <p style="font-size:12px;color:#888;margin-top:32px">
...
export function buildRecipientUnlockEmail({ writerName, readerName, triggeredByDms = false, needsKeyholder = false, forced = false }) {
  const safeWriterName = escapeHtml(writerName);
  const safeReaderName = escapeHtml(readerName);
  const safeBaseUrl = escapeHtml(BASE_URL);
  const actionText = forced
    ? '<p>The keyholders did not confirm within the grace period, so the vault has been opened automatically. You can now read the letter sections assigned to you.</p>'
    : triggeredByDms && needsKeyholder
...
<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  ...
  <hr style="border:none;border-top:1px solid #e8a84c;margin-bottom:24px"/>
	  <p>Hello ${safeReaderName},</p>
	  <p><strong>${safeWriterName}</strong> has left a sealed letter for you.</p>
  ${actionText}
  <p style="margin:32px 0">
	    <a href="${safeBaseUrl}"
       style="background:#e8a84c;color:#1a1209;padding:14px 28px;text-decoration:none;border-radius:6px;font-weight:bold;display:inline-block">
  ...
  <p style="font-size:12px;color:#888">
	    Visit <a href="${safeBaseUrl}" style="color:#e8a84c">${safeBaseUrl}</a> and enter your name
...
<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  <p style="font-size:22px;font-weight:bold;margin-bottom:4px">Letter Vault</p>
  <hr style="border:none;border-top:1px solid #e8a84c;margin-bottom:24px"/>
	  <p>A letter has been left for you by <strong>${safeWriterName}</strong>.</p>
  <p>
  ...
  <p style="font-size:12px;color:#888">
	    Or visit: <a href="${safeBaseUrl}" style="color:#e8a84c">${safeBaseUrl}</a>
  </p>
</div>`,
  };
}
