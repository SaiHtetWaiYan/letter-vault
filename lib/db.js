...
export async function getWriterData(writerId) {
  ...
  const [[writer]] = await p.execute(
    'SELECT unlock_threshold, inactivity_days, grace_days, last_active_at, warning_sent_at, dmz_unlocked_at FROM writers WHERE id = ?',
    [writerId],
  );
...
export async function touchLastActive(writerId) {
  ...
  const [[w]] = await p.execute('SELECT heartbeat_token FROM writers WHERE id = ?', [writerId]);
  const token = w?.heartbeat_token || randomUUID();
  await p.execute(
    'UPDATE writers SET last_active_at = NOW(), warning_sent_at = NULL, heartbeat_token = ? WHERE id = ?',
    [token, writerId],
  );
}

export async function setDmsConfig(writerId, { inactivityDays, graceDays }) {
  await ensureSetup();
  const p = getPool();
  await p.execute(
    'UPDATE writers SET inactivity_days = ?, grace_days = ? WHERE id = ?',
    [Math.max(1, Math.floor(inactivityDays)), Math.max(1, Math.floor(graceDays)), writerId],
  );
}
...
export async function getWriterRecipients(writerId) {
  ...
  const [rows] = await p.execute(
    'SELECT reader_name, email FROM recipients WHERE writer_id = ?',
    [writerId],
  );
...
export async function getVaultUnlockStatus(writerId) {
  ...
  if (writer?.dmz_unlocked_at) return { isUnlocked: true, confirmedTrustedCount: null, threshold: null };
  ...
  const isUnlocked = trustedCount > 0 && confirmedTrustedCount >= threshold;
...
export async function getDmsCheckList() {
  ...
  const [rows] = await p.execute(
    `SELECT w.id, w.name, w.email,
            w.inactivity_days, w.grace_days,
            w.last_active_at, w.warning_sent_at,
            w.dmz_unlocked_at, w.heartbeat_token
     FROM writers w
     WHERE w.dmz_unlocked_at IS NULL`,
  );
  return rows;
}
...
export async function createVerificationToken(writerId) {
  ...
  await getPool().execute(
    'UPDATE writers SET verification_token = ?, email_verified = 0 WHERE id = ?',
    [token, writerId],
  );
  return token;
}
...
export async function verifyEmailToken(token) {
  ...
  const p = getPool();
  const [[writer]] = await p.execute(
    'SELECT id, name, email_verified FROM writers WHERE verification_token = ?',
    [token],
  );
  if (!writer) return null;
  if (writer.email_verified) return { alreadyVerified: true, name: writer.name };
  await p.execute(
    'UPDATE writers SET email_verified = 1, verification_token = NULL WHERE id = ?',
    [writer.id],
  );
...
export async function getSectionRecipients(sectionId) {
  ...
  const [rows] = await p.execute(
    `SELECT r.reader_name, r.email
     ...
     JOIN recipients r ON lower(r.reader_name) = lower(sr.reader_name)
     WHERE sr.section_id = ?`,
    [sectionId],
  );
  return rows.map((r) => ({ readerName: r.reader_name, email: r.email || null }));
...
export async function resetHeartbeat(token) {
  ...
  await p.execute(
    'UPDATE writers SET last_active_at = NOW(), warning_sent_at = NULL WHERE heartbeat_token = ?',
    [token],
  );
  return { ok: true, name: writer.name };
}
