...
// Returns all recipients (with email if added later) for a writer — used to notify on unlock
export async function getWriterRecipients(writerId) {
  await ensureSetup();
  const p = getPool();
  const [rows] = await p.execute(
    'SELECT reader_name, passcodes_display FROM recipients WHERE writer_id = ?',
    [writerId],
  );
  return rows.map((r) => ({ readerName: r.reader_name }));
}
